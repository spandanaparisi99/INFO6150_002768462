...
class DailyWeather extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      currentDailyWeather: JSON.parse(
        localStorage.getItem("currentDailyWeather")
      ),
      ...
      <div className="rightbg widthSpacing">
        ...
        <div className="currentTemp">
          <div style={{ display: "flex" }}>
            ...
            <h2 className="degreeHead">
              {Math.round(currentDailyWeather.main.temp)}
              <sup className="supDegree">°F</sup>
            </h2>
          </div>
          <div className="speed">
            <div className="subHead">
              Humidity: {currentDailyWeather.main.humidity}%
            </div>
            <div className="subHead">
              Wind: {currentDailyWeather.wind.speed} km/h
            </div>
          </div>
          <div className="detailsHead">
            ...
            <h2 className="removeMargin">
              {currentDailyWeather.weather[0].description}
            </h2>
          </div>
        </div>
